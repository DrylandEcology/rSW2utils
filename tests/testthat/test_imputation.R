...
test_that("Impute missing values", {
  ...
  vars <- colnames(df_template0)
  vars_with_values <- vars[!apply(df_template0, 2, anyNA)]
  ...
  for (k0 in c("dataframe", "matrix")) {
    df_template <- switch(
      EXPR = k0,
      dataframe = df_template0,
      matrix = as.matrix(df_template0)
    )
